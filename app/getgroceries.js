...
//   const API = process.env.REACT_APP_API;
  ...
  const fetchOutOfStockGroceries = async () => {
    ...
    try {
      ...
      const response = await axios.get(`https://ruse-backend-1.onrender.com/groceries/outofstock`, {
  ...
  const handleOrderClick = async (grocery) => {
    try {
      const token = await AsyncStorage.getItem("token");
      await axios.delete(`https://ruse-backend-1.onrender.com/groceries/outofstock/delete`, {
  ...
  const handleLaterClick = async (grocery) => {
    try {
      const token = await AsyncStorage.getItem("token");
      await axios.put(
        `https://ruse-backend-1.onrender.com/groceries/outofstock/update`,
        { _id: grocery._id },
        { headers: { Authorization: `Bearer ${token}` } }
      );
